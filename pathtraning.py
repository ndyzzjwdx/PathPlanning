...
if __name__ == '__main__':
    ...
    file_list = ppf.get_filename(input_dir, 'train/', '.txt')
    city_list = cities.index[1::].tolist()
    for file in file_list:
        print('Loading data of ' + file + '...')
        wind_train = ppf.load_data_local(input_dir, 'train/' + file)
        wind_real = ppf.load_data_local(input_dir, 'real/' + file)
        ...
        for city in city_list:
            ...
            score, crash_flag, overtime_flag, crash = ppm.contrast(plan, wind_real, time_end)
            if crash_flag:
                print('The plane crash at (' + ppf.list_to_str(crash) + ').')
            else:
                if overtime_flag:
    ...
    # 保存提交文件
    ppf.save_file_local(plan_all.to_csv(header = False, index = False), output_dir, 'pathplanning_train.csv')
    ...
    ppf.save_file_local(out_report, output_dir, 'report_train.csv')
